...
requires(k < n) constexpr std::array<num_t, n - k> cutoff_first(
    const std::array<num_t, n> &arr) {
...
requires(first_order <= n) constexpr std::array<
    num_t, n - first_order + 1> ortho_poly(num_t x, std::array<num_t, 2> &&base,
                                           auto &&fill) {
  if constexpr (n == 0)
    return {base[0]};
  else if constexpr (n == 1 and first_order == 0)
    return {base[0], base[1]};
  else if constexpr (n == 1 and first_order == 1)
    return {base[1]};
  else {
...
requires(first_order <=
         n) constexpr std::array<std::array<num_t, n - first_order + 1>,
                                 m> ortho_poly(const std::array<num_t, m> &xs,
                                               auto &&op) {
