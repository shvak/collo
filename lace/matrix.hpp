...
namespace lace {
	...
	consteval vec_t& operator[] (std::size_t k)
	{
	    return base_t::at(k);
	}	
	...
	{
	    square_matrix res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = vec_t::basis(i);
	    return res;
	}

	static consteval square_matrix from_2darray(const std::array<std::array<num_t, n>, n>& arr)
	{
	    square_matrix res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = vec_t{arr[i]};
	    return res;
	}
	...
	consteval vec_t& row(std::size_t k)
	{
	    return base_t::at(k);
	}

	consteval const vec_t& row(std::size_t k) const
	{
	    return base_t::at(k);
	}
	...
	{
	    vec_t res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = base_t::at(i)[k];
	    return res;
	}
	...
	{
	    square_matrix res;
	    for(std::size_t i = 0; i < n; ++i) res.row(i) = col(i);
	    return res;
	}
	...
	{
	    vec_t norms;
	    for(std::size_t i = 0; i < n; ++i) norms[i] = row(i).l1norm();
	    return norms.linorm();
	}
	...
	{
	    vec_t norms;
	    for(std::size_t i = 0; i < n; ++i) norms[i] = col(i).l1norm();
	    return norms.linorm();
	}
	...
	{
	    square_matrix res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = row(i) + v;
	    return res;
	}
	...
	{
	    square_matrix res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = row(i) + m[i];
	    return res;
	}
	...
	{
	    square_matrix res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = row(i) * num;
	    return res;
	}

	friend consteval square_matrix operator* (const num_t& num, const square_matrix& m)
	{
	    return m * num;
	}
	...
	{
	    vec_t res;
	    for(std::size_t i = 0; i < n; ++i) res[i] = v * m.col(i);
	    return res;
	}
	...
	{
	    auto swap = [](const square_matrix& m, std::size_t i, std::size_t j) consteval
	    {
		square_matrix res{m};
		vec_t v = res.row(i);
		res.row(i) = res.row(j);
	    ...
	    auto upd = [](const square_matrix& m, const vec_t& v, std::size_t k) consteval
	    {
	...
	consteval square_matrix inverse() const
	{
	    ...
	    square_matrix E = identity();
	    square_matrix err = E - im * (*this);
	    while( err.norm_rows() + num_t{2.0} != num_t{2.0} or err.norm_cols() + num_t{2.0} != num_t{2.0} )
	    {
		im = (E + err + err * err) * im;
		err = E - im * (*this);
	    }
