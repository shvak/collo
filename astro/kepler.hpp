...
namespace astro{
    ...
    template<numm::number_type num_t>
    constexpr
    eccentric_anomaly_t<num_t>
    eccentric_anomaly(mean_anomaly_t<num_t> M, num_t e)
    {
	auto f = [M, e](num_t x){return mean_anomaly(eccentric_anomaly_t{x}, e) - M;};
	...
	return theta - 2 * std::atan2(beta * std::sin(theta), 1 + beta * std::cos(theta));
    }

    template<numm::number_type num_t>
    constexpr
    hyperbolic_anomaly_t<num_t>
    hyperbolic_anomaly(mean_anomaly_t<num_t> M, num_t e)
    {
	auto f = [M, e](num_t x){return mean_anomaly(hyperbolic_anomaly_t{x}, e) - M;};
    ...
    constexpr
    auto
    kepler_elements(const arr_t& xyz, num_t kappa2) // xyz: x, y, z, dot_x, dot_y, dot_z
    {
	num_t x = xyz[0], y = xyz[1], z = xyz[2],
	      dot_x = xyz[3], dot_y = xyz[4], dot_z = xyz[5];
	num_t c_1 = y * dot_z - z * dot_y,
	      c_2 = z * dot_x - x * dot_z,
	      c_3 = x * dot_y - y * dot_x,
	      c2 = c_1 * c_1 + c_2 * c_2 + c_3 * c_3,
	      c = std::sqrt(c2),
	      v2 = dot_x * dot_x + dot_y * dot_y + dot_z * dot_z,
	...
	else if(h + num_t{1.0} < num_t{1.0}) M = mean_anomaly(eccentric_anomaly(true_anomaly_t{theta}, e), e);
	else M = mean_anomaly(parabolic_sigma_t<num_t>{ std::tan(theta / num_t{2.0}) });
    ...
    constexpr
    auto
    rotate_from_orbit_plane(const arr_t& ceop, num_t i, num_t Omega, num_t g)
    {
	num_t cosi = std::cos(i),
	      sini = std::sin(i),
	      cosO = std::cos(Omega),
	      sinO = std::sin(Omega),
	      cosg = std::cos(g),
	...
	num_t x = qxx * X + qxy * Y,
	      y = qyx * X + qyy * Y,
	      z = qzx * X + qzy * Y,
	      dot_x = qxx * dot_X + qxy * dot_Y,
	      dot_y = qyx * dot_X + qyy * dot_Y,
    ...
    constexpr
    auto
    cartesian_elements_ellipse_orbit_plane(num_t p, num_t e, eccentric_anomaly_t<num_t> E, num_t kappa2)
    {
	num_t cosE = std::cos(E),
	      sinE = std::sin(E),
	      eta2 = num_t{1.0} - e * e,
	      eta = std::sqrt(eta2),
	      a = p / eta2,
	      n = std::sqrt(kappa2 / a / a / a),
    ...
    constexpr
    auto
    cartesian_elements_ellipse(const arr_t& ke, num_t kappa2) // ke: p, e, i, Omega, g, M
    {
	...
	return rotate_from_orbit_plane(
		cartesian_elements_ellipse_orbit_plane<num_t, arr_t>(p, e, E, kappa2),
		i, Omega, g);
    }
    ...
    constexpr
    auto
    cartesian_elements_hyperbola_orbit_plane(num_t p, num_t e, hyperbolic_anomaly_t<num_t> H, num_t kappa2)
    {
	num_t chH = std::cosh(H),
	      shH = std::sinh(H),
	      eta2 = e * e - num_t{1.0},
	      eta = std::sqrt(eta2),
	      a = - p / eta2,
	      n = std::sqrt(- kappa2 / a / a / a),
	      dot_H = n / (e * chH - num_t{1.0});
    ...
    constexpr
    auto
    cartesian_elements_hyperbola(const arr_t& ke, num_t kappa2) // ke: p, e, i, Omega, g, M
    {
	num_t p = ke[0], e = ke[1], i = ke[2], Omega = ke[3], g = ke[4], M = ke[5];
	...
	num_t X = p * (num_t{1.0} - sigma2) / num_t{2.0},
	      Y = p * sigma,
	      dot_X = - sigma * p_dot_sigma,
	      dot_Y = p_dot_sigma;
    ...
    constexpr
    auto
    cartesian_elements(const arr_t& ke, num_t kappa2) // ke: p, e, i, Omega, g, M
    {
