...
namespace collo
{
	...
	protected:
	    ...
	    using sva_t = Eigen::Matrix<num_t, system_order, method_stage>; // type of state_vector's array

	    consteval static auto shifted_tn(const std::array<num_t, method_stage>& time_nodes, int shift = 0)
	    {
		using lacev = lace::vector<num_t, method_stage>;

		if(shift == 0) return time_nodes;
		auto tn = lacev{time_nodes};
		auto tn_sh = tn + static_cast<num_t>(shift);
		return tn_sh.to_array();
	    }

	    consteval static auto make_sv_nodes(const std::array<num_t, method_stage>& time_nodes, int shift = 0)
	    {
		using lacev = lace::vector<num_t, method_stage>;
		using lacem = lace::square_matrix<num_t, method_stage>;

		auto sv_nodes = lacem::from_2darray(numm::legendre_sh<method_stage, 1>( shifted_tn(time_nodes, shift) ));
	    ...
	    consteval static auto make_lsm(const std::array<num_t, method_stage>& time_nodes)
	    {
		using lacem = lace::square_matrix<num_t, method_stage>;

		auto lsm = lacem::from_2darray(numm::dlegendre_sh<method_stage, 1>( time_nodes ));
	...
	private:
	    ...
	    auto time_point(std::size_t i) const
	    {
		return base_t::tp(time(), h, i);
	    }
    ...
    struct Predictor_Base : protected base_t
    {
	...
	protected:
	    ...
	    static auto time_node(std::size_t i)
	    {
	...
	protected:
	    ...
	    Predictor_PrevStep()
		: alphas( base_t::sva_t::Zero() )
	    {}

	    auto make_alphas(const base_t::sv_t&, const auto&, const auto&, auto) const
	    {
		return alphas;
	    }
	...
	protected:
	    ...
	    static auto euler(const base_t::sv_t& y, const auto& t, const auto& h, auto rhs)
	    {
		if(h == 0) return y;
	...
	protected:
	    ...
	    static auto rk4(const base_t::sv_t& y, const auto& t, const auto& h, auto rhs)
	    {
		if(h == 0) return y;
	...
	private:
	    ...
	    constexpr static auto pred_sv_nodes = base_t::make_sv_nodes(base_t::time_nodes, 1);
	...
	protected:
	    ...
	    Predictor_Poly()
		: alphas( base_t::sva_t::Zero() ), y{}, first_step{true}
	    {}

	    auto make_alphas(const base_t::sv_t&, const auto& t, const auto& h, auto rhs) const
	    {
		if(first_step)
		{
		    return alphas;
	...
	protected:
		...
		if(lim == 0) return base_t::sva_t::Zero().eval();
	...
	using type = std::conditional_t<
		     ...
		     std::conditional_t<
			 p == Pred::Poly,
		         Predictor_Poly<base_t<num_t, system_order, method_stage>>, 
		     std::conditional_t<
			 p == Pred::BackDiff,
		         Predictor_BackDiff<base_t<num_t, system_order, method_stage>, param>,
			 void
		     >>>>>>>; 
    };
